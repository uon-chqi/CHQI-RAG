...
export default function Chat() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (phone.length >= 8 && ccc.length >= 3) {
      setIsLoggedIn(true);
    } else {
      setLoginError('Invalid phone or CCC number.');
    }
  };
        ...
        if (!isLoggedIn) {
          return (
            <div className="flex flex-col items-center justify-center min-h-[70vh]">
              <form onSubmit={handleLogin} className="bg-white p-8 rounded-lg shadow-md w-full max-w-xs flex flex-col gap-4">
                <h2 className="text-xl font-bold text-center">Chat Login</h2>
                <input
                  type="tel"
                ...
                <button type="submit" className="bg-green-600 text-white rounded py-2 font-semibold hover:bg-green-700">Login</button>
              </form>
            </div>
          );
        }
